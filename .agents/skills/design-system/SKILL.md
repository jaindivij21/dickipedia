...
- On home/index, the eyebrow + rule IS the section header; no icon chips there. On the
  data-dense MP detail page, a section header pairs the mono `.eyebrow` kicker with a serif
  H2 title and a single hairline-framed (`rounded-lg` `border-border` `bg-surface-2`) icon
  tile — an intentional richness affordance; keep section icons monochrome ink, one per
  section. Elsewhere icons stay inline in flags, bars, stat-tile labels, and margin rows.
...
- A zone heading MUST hang off a 2px ink top rule: `.rule-top` + `pt-3` (eyebrow) or
  ...
  or a rounded card.
- Inter-zone spacing is generous: `mt-16 sm:mt-20` (home), `mt-14` (detail sections). Detail
  section content may sit in `rounded-lg` `border-border` panels and separated `rounded-lg`
  stat tiles (`grid gap-3`, hover `bg-surface-2`); the header still hangs off the 2px rule.
...
- Never use `--color-accent` to mean "bad" — bad is `danger`; accent is editorial emphasis,
  ...
  figure. On home/index a zone spends at most ONE resting accent; the MP detail page also uses
  a small `bg-accent` rule tick (`h-0.5 w-10`) at the start of each section rule as a
  periodical motif plus one `.mark-accent` standfirst phrase — data meaning still comes only
  from the bands.
...
- ❌ `--color-accent` used for data meaning, `colorVar('accent')`. On home/index, >1 resting
  accent/zone (the detail page's per-section rule tick is the sanctioned exception).
