...
const num = (s: string | undefined): number => Number(String(s ?? "").replace(/[^0-9.-]/g, "")) || 0;
...
async function main(): Promise<void> {
  ...
  const spine: EciSpineRow[] = winners.map((w) => {
    ...
    const winnerRow = nonNota.find((c) => normName(c["Candidate"]) === normName(w["Winning Candidate"]));
    ...
    return {
      ...
      _provenance: { source: "ECI via opencity.in CKAN", license: "Public Domain", url: WINNERS_CSV },
    };
  });
