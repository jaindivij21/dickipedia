...
async function main(): Promise<void> {
  console.log("Fetching MyNeta winners index (ADR/MyNeta.info, from ECI affidavits; non-commercial, attributed)...");
  ...
  $("table.w3-table tr").each((_, tr) => {
    ...
    rows.push({
      ...
      name, name_norm: normName(name),
      constituency, constituency_norm: normConstituency(constituency),
      ...
      _provenance: { source: "ADR / MyNeta.info (from ECI affidavits)", license: "Non-commercial; display + attribution", url: INDEX_URL },
    });
  });
  ...
  const richest = rows.filter((r) => r.total_assets != null).sort((a, b) => b.total_assets - a.total_assets)[0];
  ...
  if (richest) console.log(`Richest: ${richest.name} (${richest.constituency}) — Rs ${richest.total_assets.toLocaleString("en-IN")}`);
