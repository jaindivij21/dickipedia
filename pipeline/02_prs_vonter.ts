...
async function main(): Promise<void> {
  console.log("Fetching PRS/Vonter 18th LS activity (ODbL-1.0; attribute PRS Legislative Research)...");
  ...
  const out: PrsRow[] = rows.map((r) => {
    ...
    for (const [k, v] of Object.entries(r)) {
      ...
      if (m) { const val = num(v); if (val) ministry[m[1]] = val; }
    }
    ...
    return {
      ...
      _provenance: { source: "PRS Legislative Research via Vonter mirror", license: "ODbL-1.0", url: PRS_CSV },
    };
  });
  ...
  const avgAtt = Math.round((nonMin.reduce((s, m) => s + (m.attendance_pct || 0), 0) / (nonMin.length || 1)) * 10) / 10;
  ...
  console.log(`Zero-question MPs (non-ministers): ${nonMin.filter((m) => m.questions === 0).length}`);
