...
export async function run(): Promise<void> {
  ...
  for (const r of enc) {
    ...
    cur.total += rupee(r['denomination']);
    cur.count += 1;
  ...
  const ranked = [...party.entries()]
    .map(([short, v]) => ({
      ...
      bond_total: v.total,
      bond_count: v.count,
      top_donors: [...(donors.get(short)?.entries() || [])]
        .sort((a, b) => b[1] - a[1])
        .slice(0, 5)
  ...
  await writeFile(
    new URL('electoral_bonds.json', RAW),
    JSON.stringify(
      {
        _provenance: {
          source: 'ECI/SBI electoral-bond disclosure (SC: ADR v Union of India, 15 Feb 2024)',
  ...
  for (const p of ranked.slice(0, 5))
    console.log(
      `  ${p.rank}. ${p.party}: Rs ${p.bond_total.toLocaleString('en-IN')} (${p.bond_count} bonds)`,
    );
