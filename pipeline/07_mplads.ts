...
const intOf = (s: unknown): number => { const m = String(s ?? "").match(/\d+/); return m ? Number(m[0]) : 0; };

async function main(): Promise<void> {
  const ax = axios.create({ headers: { "User-Agent": UA }, timeout: 60000 });
  const dash = await ax.get(DASH);
  const cookie = ((dash.headers["set-cookie"] as string[]) || []).map((c) => c.split(";")[0]).join("; ");
  const post = async (ep: string, body: any): Promise<any> => {
    const r = await ax.post(`${BASE}/${ep}`, body, {
      headers: { "Content-Type": "application/json", Accept: "application/json", Referer: DASH, Cookie: cookie },
    });
    return r.data;
  };
  ...
  for (const st of states) {
    ...
    si++;
    const mps: { ID: number; CAPTION: string }[] = await post("getMpNamesData", { state_combo: `${st.STATE_ID},2,7` });
    await sleep(400);
    for (const mp of mps) {
      if (done.has(mp.ID)) continue;
      let t: any;
      try { t = await post("getTilesData", { uname: `${st.STATE_ID},0,${mp.ID},2` }); }
      catch (e: any) { console.error("tiles fail", mp.ID, e.message); await sleep(1500); continue; }
      const alloc = rupee(t["Allocated Limit for"]?.[0]);
      const exp = rupee(t["Expenditure on Completed and On-going Works as on Date"]?.[0]);
      out.push({
        mpId: mp.ID, name: mp.CAPTION, name_norm: normName(mp.CAPTION), state: st.STATE_NAME,
        allocated: alloc, expenditure: exp,
  ...
  const low = out.filter((o) => o.utilisation_pct != null).sort((a, b) => a.utilisation_pct - b.utilisation_pct)[0];
  console.log(`MPLADS MPs: ${out.length}`);
  if (low) console.log(`Lowest fund utilisation: ${low.name} (${low.state}) — ${low.utilisation_pct}% spent`);
