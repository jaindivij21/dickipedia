...
const num = (s: string): number | null => { const m = String(s).match(/([0-9][0-9,]{2,})/); return m ? Number(m[1].replace(/,/g, "")) : null; };
const pct = (s: string): number | null => { const m = String(s).match(/-?[\d.]+/); return m ? Number(m[0]) : null; };
...
async function main(): Promise<void> {
  ...
  $("table tr").each((_, tr) => {
    ...
    let w = byName.get(nn);
    if (!w) {
      let best: any = null, bc = 0;
      for (const cand of winners) { const c = tokenSortRatio(name, cand.name); if (c > bc) { bc = c; best = cand; } }
      if (bc >= 90) w = best;
    }
    rows.push({
      name, name_norm: nn,
      ...
      _provenance: { source: "ADR/MyNeta.info recontestAssetsComparison (from ECI affidavits)", license: "Non-commercial; display + attribution", url: URL_ },
    });
  });
  ...
  const top = rows.filter((r) => r.pct_increase != null).sort((a, b) => b.pct_increase - a.pct_increase)[0];
  console.log(`Wealth-growth rows: ${rows.length}; joined to a constituency: ${matched}`);
  if (top) console.log(`Biggest % jump: ${top.name} +${top.pct_increase}% (Rs ${(top.assets_2019 ?? 0).toLocaleString("en-IN")} -> Rs ${(top.assets_2024 ?? 0).toLocaleString("en-IN")})`);
