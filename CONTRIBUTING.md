...
dickipedia is an open, public-interest accountability record for India's elected Lok Sabha MPs (18th Lok Sabha, 2024). The whole project rests on one rule:
...
- **Code:** fork → branch → PR. The TypeScript pipeline lives in `pipeline/`; `npm run eci | prs | sansad | canonical` rebuilds `data/canonical/mps.json`.
...
- Electoral-bond funding is **party-level**, never pinned to an individual MP.
