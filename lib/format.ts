...
export function scoreBand(v: number | null | undefined): { token: "danger" | "warning" | "success"; label: string } {
  if (v == null) return { token: "warning", label: "Not enough data" };
  if (v < 34) return { token: "danger", label: "Poor" };
  if (v < 60) return { token: "warning", label: "Mediocre" };
  return { token: "success", label: "Decent" };
}

export const colorVar = (token: "danger" | "warning" | "success" | "primary") => `var(--color-${token})`;
