          ...
          <div className='flex flex-col gap-2'>
            {donors.map((d) => (
              <div key={d.name} className='flex items-center gap-2 sm:gap-3'>
                <span className='text-ink-soft w-28 shrink-0 truncate font-mono text-[11px] sm:w-48'>
                  {d.name}
                </span>
                <Bar value={d.amount} max={donorMax} color='var(--color-ink)' />
                <span className='w-16 shrink-0 text-right font-mono text-[11px] font-bold sm:w-20'>
                  {rupeeCr(d.amount)}
                </span>
              </div>
            ))}
